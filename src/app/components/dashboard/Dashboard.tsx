...
const HUB_CONFIG: Record<string, any> = {
  drawing: {
    ...
    kpis: [
      { label: 'Drawings', value: '124', change: 12, trend: 'up', icon: FileText, color: 'text-blue-600', bg: 'bg-blue-50' },
      { label: 'Accuracy', value: '98.2%', change: 2, trend: 'up', icon: ShieldCheck, color: 'text-indigo-600', bg: 'bg-indigo-50' },
      { label: 'Pending', value: '8', change: -5, trend: 'down', icon: Clock, color: 'text-gray-400', bg: 'bg-gray-50' },
      { label: 'Verified', value: '856', change: 18, trend: 'up', icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    ],
  ...
  bom: {
    title: 'Data System',
    kpis: [
      { label: 'Tables', value: '452', change: 15, trend: 'up', icon: Database, color: 'text-emerald-600', bg: 'bg-emerald-50' },
      { label: 'Integrity', value: '99.8%', change: 1, trend: 'up', icon: ShieldCheck, color: 'text-indigo-600', bg: 'bg-indigo-50' },
      { label: 'Confidence', value: '94.5%', change: 5, trend: 'up', icon: Target, color: 'text-blue-600', bg: 'bg-blue-50' },
      { label: 'BOM Items', value: '12.4k', change: 22, trend: 'up', icon: Activity, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    ],
  ...
  visual: {
    title: 'Visual System',
    kpis: [
      { label: 'Images', value: '78', change: 8, trend: 'up', icon: ImageIcon, color: 'text-amber-600', bg: 'bg-amber-50' },
      { label: 'Accuracy', value: '92.1%', change: 3, trend: 'up', icon: Target, color: 'text-indigo-600', bg: 'bg-indigo-50' },
      { label: 'Active', value: '14', change: 2, trend: 'up', icon: Activity, color: 'text-blue-600', bg: 'bg-blue-50' },
      { label: 'Matches', value: '342', change: 12, trend: 'up', icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    ],
  ...
  text: {
    ...
    kpis: [
      { label: 'Emails', value: '1.2k', change: 18, trend: 'up', icon: Mail, color: 'text-indigo-600', bg: 'bg-indigo-50' },
      ...
      { label: 'Answers', value: '156', change: 15, trend: 'up', icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    ],
...
const statusData = [
  ...
  { name: 'Draft', value: 15, color: '#6366f1' },
    ...
    <div className="min-h-screen bg-[#F8F9FB] pb-32">
      ...
      <div className="space-y-10">
            ...
            {config.kpis.map((kpi: any) => (
              <div key={kpi.label} className="bg-white p-8 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between h-44">
                <div className="flex justify-between items-start">
                   ...
                   <div className={`p-3 rounded-lg ${kpi.bg} ${kpi.color} shadow-sm`}>
        ...
        <div className="bg-white p-12 rounded-xl border border-gray-100 shadow-sm relative overflow-hidden group">
           <div className={`absolute top-0 right-0 w-64 h-64 ${config.kpis[0].bg} rounded-full translate-x-1/2 -translate-y-1/2 opacity-20 transition-transform group-hover:scale-110`} />
           <div className="relative z-10">
              ...
              <p className="text-[15px] text-gray-500 mt-3 max-w-3xl leading-relaxed font-normal">
                 You are in the **{config.title}**. This system helps you with your {config.id === 'drawing' ? 'drawings' : 'data'} work.
              </p>
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-10">
           <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 p-10 shadow-sm">
              <h3 className="text-sm font-bold text-gray-900 uppercase tracking-widest mb-10 decoration-indigo-500/30 underline decoration-4 underline-offset-8">{config.chartLabel}</h3>
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={chartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="rfqs" fill={config.kpis[0].bg.replace('bg-', '').replace('-50', '-600')} radius={[4, 4, 0, 0]} barSize={42} />
                </BarChart>
              </ResponsiveContainer>
           </div>
