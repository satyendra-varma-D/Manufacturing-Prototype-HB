...
const PRICING_STRATEGIES = [
  ...
  { id: 'standard', label: 'Standard', margin: 0.20, description: 'Default operating margin', color: 'text-primary', dot: 'bg-primary' },
    ...
    <div className="max-w-[1400px] mx-auto space-y-10 pb-32 pt-10 px-8 bg-[#fafbfc] min-h-screen">
      ...
      <div className="flex items-center justify-between bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={handleSubmit} className="px-6 py-3 bg-primary text-white font-bold rounded-xl shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all text-xs flex items-center gap-2.5 active:scale-95">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 space-y-8">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-4">
              {PRICING_STRATEGIES.map((strategy) => (
                <button key={strategy.id} type="button" onClick={() => setSelectedStrategy(strategy)} className={`p-6 rounded-xl border transition-all text-left group relative ${selectedStrategy.id === strategy.id ? 'border-primary bg-white ring-2 ring-primary/5 shadow-md' : 'border-gray-100 hover:border-gray-200 bg-white'}`}>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-8 shadow-sm">
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-2">
                <button type="button" onClick={handleAISuggest} className="flex items-center gap-2 px-4 py-2 bg-secondary text-primary rounded-lg hover:bg-primary hover:text-white text-xs font-bold transition-all border border-primary/20"><Sparkles className="w-3.5 h-3.5" /> AI Optimizer</button>
                    ...
                    <div className="md:col-span-3 flex items-end gap-3">
                      <div className="flex-1">
                        ...
                        <div className="relative">
                          ...
                          <input type="number" value={item.unitPrice} onChange={(e) => handleItemChange(item.id, 'unitPrice', parseFloat(e.target.value) || 0)} className="w-full pl-8 pr-4 py-2.5 bg-gray-50/50 rounded-lg border border-transparent focus:bg-white focus:border-primary/40 outline-none transition-all font-bold text-gray-900 text-xs" />
                        </div>
                      </div>
          ...
          <div className="bg-primary rounded-xl p-8 text-white flex items-center justify-between shadow-xl shadow-primary/10 transition-all">
            <div className="flex items-center gap-6">
              ...
              {[
                ...
                <div key={i} className="relative flex items-center gap-4">
                  <div className={`w-5.5 h-5.5 rounded-full z-10 flex items-center justify-center border-4 border-white shadow-sm font-bold text-[9px] ${step.status === 'done' ? 'bg-emerald-500 text-white' :
                    step.status === 'current' ? 'bg-primary text-white animate-pulse' : 'bg-gray-100 text-gray-400'
                    }`}>
